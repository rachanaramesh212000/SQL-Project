...
SELECT region_id,COUNT(DISTINCT node_id) AS nodes_per_region FROM customer_nodes GROUP BY region_id ORDER BY region_id;
...
select avg(DATEDIFF(end_date, start_date)) AS avg_days_allocated from customer_nodes
where end_date IS NOT NULL;

#5.What is the median, 80th and 95th percentile for this same reallocation days metric for each region?
select region_id,
       min(case when tile = 50 then days_allocated end) as median_days,
       min(case when tile = 80 then days_allocated end) as p80_days,
       min(case when tile = 95 then days_allocated end) as p95_days
from (
    select region_id,DATEDIFF(end_date, start_date) as days_allocated,
           NTILE(100) over (partition by region_id order by DATEDIFF(end_date, start_date)) as tile
    from customer_nodes
    where end_date is not null) t group by region_id order by region_id;
    
    #B. Customer Transactions
    
    #1.Unique count & total amount for each transaction type
    select txn_type,count(distinct customer_id) as unique_customers,sum(txn_amount) as total_amount from customer_transactions
     group by txn_type;
     
    #2. Average total historical deposit counts & amounts for all customers
    select avg(deposit_count) as avg_deposit_count,avg(deposit_amount) as avg_deposit_amount FROM (
    select customer_id,COUNT(*) as deposit_count,SUM(txn_amount) as deposit_amount from customer_transactions
    where txn_type = 'deposit'
    group by customer_id)sub;
    
    #3.For each month - how many Data Bank customers make more than 1 deposit and either 1 purchase or 1 withdrawal in a single month?
    
   select month,count(distinct customer_id) as qualifying_customers from (
    select customer_id,date_format(txn_date, '%Y-%m') as month,
        ...
        sum(case when txn_type = 'withdrawal' then 1 else 0 end) as withdrawals from customer_transactions
    group by customer_id, date_format(txn_date, '%Y-%m')
    having deposits > 1 and (purchases >= 1 or withdrawals >= 1)) t group by month order by month;
    
    
    #4.Closing balance for each customer at the end of each month
select customer_id, date_format(txn_date, '%Y-%m-01') as month,
       ...
       sum(sum(case when txn_type='deposit' then txn_amount else -txn_amount end))
           over(partition by customer_id order by date_format(txn_date, '%Y-%m-01')) as closing_balance
...
select customer_id, date(txn_date) as day,
       avg(sum(case when txn_type='deposit' then txn_amount else -txn_amount end)) over (partition by customer_id order by date(txn_date) rows 29 preceding) as avg_balance_30days
from customer_transactions
...
select customer_id, txn_date, txn_type, txn_amount,
       sum(case when txn_type='deposit' then txn_amount else -txn_amount end)
           over(partition by customer_id order by txn_date, txn_type, txn_amount) as running_balance
from customer_transactions
order by customer_id, txn_date;

#2.customer balance at the end of each month
select customer_id, extract(year from txn_date) as year, extract(month from txn_date) as month, 
       sum(case when txn_type = 'deposit' then txn_amount else -txn_amount end) as balance
from customer_transactions group by customer_id, extract(year from txn_date), extract(month from txn_date);

#3.minimum,avg,max values of running balance for each customer
with running_balances as (
  select customer_id, 
         sum(case when txn_type = 'deposit' then txn_amount else -txn_amount end) 
         over (partition by customer_id order by txn_date) as running_balance from customer_transactions)
select customer_id,
        min(running_balance) as min_balance,
       avg(running_balance) as avg_balance,
       max(running_balance) as max_balance from running_balances group by customer_id;
...
#1. Data Bank wants an initial calculation which does not allow for compounding interest, however they may also be interested in a daily 
...
select customer_id, txn_date,
       ...
       (sum(case when txn_type='deposit' then txn_amount else -txn_amount end)
           over(partition by customer_id order by txn_date)) * 0.05 / 365 as daily_interest
from customer_transactions
order by customer_id, txn_date;
